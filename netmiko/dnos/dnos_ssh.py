...
class DnosSSH(BaseConnection):
    ...
    def check_enable_mode(self, check_string: str = "#") -> bool:
        ...
        # overriding the enable mode method, always return true as enable mode is not available in dnos
        return True
    ...
    def enable(
        ...
        """Enter enable mode."""
        # overriding the enable mode method, do nothing as enable mode is not available in dnos
        return ""
    ...
    def find_prompt(
        ...
        """
        Updates the find_prompt to workaround the default prompt on the DNOS shell.
        """
        prompt = super().find_prompt(delay_factor=delay_factor, pattern=pattern)
        # regex to filter out the date, time and namespace from the prompt in the container shell
        date_time_ns_regex = "\/\[.*\]\[.*\]"
    ...
    def _enter_shell(self) -> str:
        """Enter the Bourne Shell of the routing-engine container on the active NCC"""
